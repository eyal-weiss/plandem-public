...
EvaluationContext::EvaluationContext(
    ...
    bool calculate_preferred, EstimationInfo *g_estimation_ptr)
    : cache(cache),
      state(state),
      g_value(g_value),
      ...
      calculate_preferred(calculate_preferred) {
          if (g_estimation_ptr != nullptr) {
              g_estimation.min_g = g_estimation_ptr->min_g;
              g_estimation.max_g = g_estimation_ptr->max_g;
              g_estimation.rank = g_estimation_ptr->rank;
              g_estimation.try_next = g_estimation_ptr->try_next;
          }
}
...
const EstimationInfo &EvaluationContext::get_estimation_info() {
    return g_estimation;
}
